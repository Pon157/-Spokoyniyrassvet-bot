class SettingsManager {
    ...
    constructor() {
        ...
        // ВРЕМЕННОЕ РЕШЕНИЕ: отключаем проверку аутентификации
        console.log('⚡ SettingsManager: временно отключаем проверку аутентификации');
        this.initWithoutAuth();
    }

    // Временный метод для тестирования
    initWithoutAuth() {
        console.log('🔓 Загрузка без аутентификации...');
        ...
        if (userData) {
            try {
                ...
                console.log('✅ Данные пользователя из localStorage:', this.currentUser);
            } catch (e) {
                console.error('❌ Ошибка парсинга user_data:', e);
            }
        }
        
        // Если нет данных, создаем временного пользователя
        if (!this.currentUser) {
            this.currentUser = {
            ...
            console.log('👤 Создан временный пользователь:', this.currentUser);
        }
        ...
        console.log('✅ Панель настроек загружена в демо-режиме');
    }
        ...
        const notifications = [
            {
                id: 'notif-1',
                title: 'Демо-режим',
                message: 'Панель настроек работает в демо-режиме. Все изменения сохраняются локально.',
                is_read: false,
                created_at: new Date().toISOString()
            }
        ];
    ...
    // Переопределяем методы сохранения для работы без сервера
    async saveProfile() {
        ...
        this.showNotification('Профиль обновлен (демо-режим)', 'success');
    }

    async saveSettings() {
        this.showNotification('Настройки сохранены (демо-режим)', 'success');
    }

    async saveNotificationSettings() {
        this.showNotification('Настройки уведомлений сохранены (демо-режим)', 'success');
    }

    // ... остальные методы из предыдущей версии

    logout() {
        console.log('🚪 Выход из демо-режима...');
